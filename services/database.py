...
class Database:
    ...
    def _gen_object(self, model: type, fields: tuple):
        ...
        mapping = {
            schema[index]: fields[index] for index in range(len(fields))
        }
    ...
    def _gen_insert_query(self, model: type, model_instance, id: int | None = None):
        ...
        if id is not None:
            ...
            values.append(id)
            return f"UPDATE {model.table_name()} SET {set_clause} WHERE id = ?", tuple(values)
        return f"INSERT INTO {model.table_name()} ({columns}) VALUES ({placeholders})", tuple(values)
    ...
    def update(self, model_instance):
        with self.get_db() as conn:
            ...
            query, values = self._gen_insert_query(type(model_instance), model_instance, id)
            cursor.execute(query, values)
            conn.commit()
            return model_instance
    ...
    def delete(self, model_instance) -> bool:
        with self.get_db() as conn:
            cursor = conn.cursor()
            cursor.execute(f"DELETE FROM {model_instance.table_name()} WHERE id=?", (model_instance.id,))
            conn.commit()
            return True
